...
public class ConnectPlugin extends CordovaPlugin {
    ...
    CallbackContext initIsGoodCollback;
    class CallbackClass implements FacebookSdk.InitializeCallback{

        ConnectPlugin plugin;

        CallbackClass(ConnectPlugin target){
            plugin = target;
        }

        @Override
        public void onInitialized() {
            //Log.i("CLZ", "Override onInitialized ");
            FacebookSdk.fullyInitialize();
            plugin.defaultPluginInitialize();
        }
    }
    ...
    protected void defaultPluginInitialize() {
    ...
    public boolean execute(String action, JSONArray args, final CallbackContext callbackContext) throws JSONException {
        if (action.equals("initialize")) {
            executeInitialize(args, callbackContext);
            return true;
    ...
    private void executeInitialize(JSONArray args, CallbackContext callbackContext) throws JSONException {
        Log.i("CLZ", "executeInitialize");
        initIsGoodCollback = callbackContext;
        FacebookSdk.setApplicationId(args.getString(0));
        CallbackClass callback = new CallbackClass(this);
        FacebookSdk.sdkInitialize(cordova.getActivity().getApplicationContext(),callback);

        for (int i = 0; i < args.length(); i++) {
            //Log.i("CLZ", "args = " + args.getString(i));
        }
    }
